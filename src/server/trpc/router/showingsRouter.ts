...
export const showingsRouter = t.router({
    getAllGroups: authedProcedure.input(
        z.object({ 
    ...
    getAllByWorkspace: authedProcedure.input(
        z.object({ 
            workspaceId: z.string()
        }))
        .query(async ({ctx, input}) => {
    ...
    getAllByGroup: authedProcedure.input(
        z.object({ 
    ...
    getOne: authedProcedure.input(
        ...
        .query(async ({ctx, input}) => {
            return await ctx.prisma.showing.findUnique({where: {
                id: input.id
            }})
        }),
    createGroup: authedProcedure.input(z.object({
        title: z.string(),
        workspaceId: z.string(),
        date: z.string()
    }))
