...
export const AppointmentsModal = () => {
    const { modalOpen, setModalOpen } = useAppointmentsUI();
    ...
    /*  const { mutate, isLoading } = trpc.appointment.createGroup.useMutation({
        onSuccess(data) {
            router.push(`/workspace/${data.workspaceId}/appointments/${data.id}`);
            utils.appointment.getAllGroups.invalidate({
